  ...
  module IntegrationTestMethods
    ...
    def assert_cache_pages(*urls)
      ...
      urls.each do |url|
		...
		alias :assert_cache_page :assert_cache_pages

	# asserts that the list of given url are NOT being cached
    def assert_not_cache_pages(*urls)
      ActionController::Base.reset_cache
      ...
      urls.each do |url|
		...
		alias :assert_not_cache_page :assert_not_cache_pages
    ...
    def assert_expire_pages(*urls)
      ...
      urls.each do |url|
		...
		alias :assert_expire_page :assert_expire_pages
		
		# asserts that the list of given url are NOT being expired
    def assert_not_expire_pages(*urls)
      ActionController::Base.reset_cache
      
      yield *urls

      urls.each do |url|
        assert_block("#{url.inspect} was expired after executing block") do
          !ActionController::Base.expired?(url)
        end
      end
    end

		alias :assert_not_expire_page :assert_not_expire_pages
  end
end